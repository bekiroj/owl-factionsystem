addCommandHandler('setfaction', function(thePlayer, commandName, partialNick, factionID)
	if exports.slua_integration:isPlayerSeniorAdmin(thePlayer) then
		local factionID = tonumber(factionID)
		if not (partialNick) or not (factionID) then
			thePlayer:outputChat('['..exports.config:getServerName()..']:#D0D0D0 /'..commandName..' [Target ID] [faction ID]',195,184,116,true)
		else
			local targetPlayer, targetPlayerNick = exports.slua_global:findPlayerByPartialNick(thePlayer, partialNick)
			if targetPlayer then
				local theTeam = exports.slua_pool:getElement("team", factionID)
				if not theTeam and factionID ~= -1 then
					thePlayer:outputChat('['..exports.config:getServerName()..']:#D0D0D0 Hatalı faction ID!',195,184,116,true)
					return
				end
				if dbExec(connection:getConnection(),"UPDATE characters SET faction_leader = 0, faction_id = " .. factionID .. ", faction_rank = 1, faction_phone = NULL, duty = 0 WHERE id=" .. targetPlayer:getData('dbid')) then
					if factionID > 0 then
						...
						thePlayer:outputChat('['..exports.config:getServerName()..']:#D0D0D0 Başarıyla '..targetPlayer.name..' isimli oyuncuyu '..factionID..' ID birliğe atadınız!',195,184,116,true)
						targetPlayer:outputChat('['..exports.config:getServerName()..']:#D0D0D0 '..thePlayer.name..' isimli yetkili sizi '..factionID..' ID birliğe atadı!',195,184,116,true)
						exports.slua_logs:dbLog(thePlayer, 4, { targetPlayer, thePlayer }, 'SET TO FACTION')
					else
						...
						targetPlayer:setData('factionphone', nil, true)
						targetPlayer:setData('factionleader', 0, true)
						thePlayer:outputChat('['..exports.config:getServerName()..']:#D0D0D0 Başarıyla '..targetPlayer.name..' isimli oyuncuyu birliğinden çıkardınız!',195,184,116,true)
						targetPlayer:outputChat('['..exports.config:getServerName()..']:#D0D0D0 '..thePlayer.name..' isimli yetkili sizi birliğinizden çıkardı!',195,184,116,true)
						exports.slua_logs:dbLog(thePlayer, 4, { targetPlayer, thePlayer }, 'REMOVE FROM FACTION')
					end
				else
					thePlayer:outputChat('['..exports.config:getServerName()..']:#D0D0D0 Bir hata meydana geldi!',195,184,116,true)
				end
			end
		end
	end
			...
			if targetPlayer then
				...
				if dbExec(connection:getConnection(),"UPDATE characters SET faction_leader = 1, faction_id = " .. factionID .. ", faction_rank = 1, faction_phone = NULL, duty = 0 WHERE id=" .. targetPlayer:getData('dbid')) then
					if factionID > 0 then
						...
						thePlayer:outputChat('['..exports.config:getServerName()..']:#D0D0D0 Başarıyla '..targetPlayer.name..' isimli oyuncuyu '..factionID..' ID birliğe lider atadınız!',195,184,116,true)
						targetPlayer:outputChat('['..exports.config:getServerName()..']:#D0D0D0 '..thePlayer.name..' isimli yetkili sizi '..factionID..' ID birliğe lider atadı!',195,184,116,true)
						exports.slua_logs:dbLog(thePlayer, 4, { targetPlayer, thePlayer }, 'SET TO FACTION')
					else
						local citizenTeam = Team.getFromName('Citizen')
						targetPlayer.team = citizenTeam
...
addCommandHandler('birlikonayla', function(thePlayer, commandName, factionID)
	if exports.slua_integration:isPlayerDeveloper(thePlayer) then
		if tonumber(factionID) then
			...
			thePlayer:outputChat('['..exports.config:getServerName()..']:#D0D0D0 /'..commandName..' [Birlik ID]',195,184,116,true)
		end
	end
end)
...
addCommandHandler('renamefaction', function(thePlayer, commandName, ...)
	if thePlayer:getData('factionleader') == 1 then
		if not (...)  then
			thePlayer:outputChat('['..exports.config:getServerName()..']:#D0D0D0 /'..commandName..' [Yeni İsim]',195,184,116,true)
		else
			if string.len(...) > 16 then
				thePlayer:outputChat('['..exports.config:getServerName()..']:#D0D0D0 Birlik adı çok uzun olmamalı!',195,184,116,true)
			return end
			...
			if owner == playerDB then
				if playerFact > 0 then
					if dbExec(connection:getConnection(), "UPDATE vehicles SET faction='" .. (playerFact) .. "' WHERE id='" .. (vehicleDB) .. "'") then
						exports["slua_vehicle"]:reloadVehicle(vehicleDB)
						thePlayer:outputChat('['..exports.config:getServerName()..']:#D0D0D0 Aracınız bulunduğunuz birliğe eklendi.',195,184,116,true)
					else
						thePlayer:outputChat('['..exports.config:getServerName()..']:#D0D0D0 Bir hata meydana geldi!',195,184,116,true)
					end
				else
					thePlayer:outputChat('['..exports.config:getServerName()..']:#D0D0D0 Herhangi bir birlikte değilsiniz!',195,184,116,true)
				end
			end
		end
	end
			...
			if owner == playerDB then
				if vehicleFact > 0 then
					if dbExec(connection:getConnection(), "UPDATE vehicles SET faction='-1' WHERE id='" .. (vehicleDB) .. "'") then
						exports["slua_vehicle"]:reloadVehicle(vehicleDB)
						thePlayer:outputChat('['..exports.config:getServerName()..']:#D0D0D0 Aracınız bulunduğu birlikten çıkarttınız.',195,184,116,true)
					else
						thePlayer:outputChat('['..exports.config:getServerName()..']:#D0D0D0 Bir hata meydana geldi!',195,184,116,true)
					end
				else
					thePlayer:outputChat('['..exports.config:getServerName()..']:#D0D0D0 Aracınız zaten bir birlikte değil!',195,184,116,true)
				end
			end
		end
	end
		...
		if (...) then
			...
			if playerFact <= 0 then
				if string.len(...) > 16 then
					thePlayer:outputChat('['..exports.config:getServerName()..']:#D0D0D0 Birlik adı çok uzun olmamalı!',195,184,116,true)
				return end
				if exports.slua_global:takeMoney(thePlayer, 10000) then
					...
					if theTeam then
						if dbExec(connection:getConnection(), "INSERT INTO factions SET id='"..(id).."', name='"..(name).."', bankbalance='0', type='0'") then
								...
								function(qh)
									...
									if rows > 0 then
										for index, row in ipairs(res) do
											...
											exports.slua_pool:allocateElement(theTeam, id)
							...
							thePlayer:outputChat('['..exports.config:getServerName()..']:#D0D0D0 10.000'..exports.config:getServerMoneyType()..' ödeyerek birlik oluşturdunuz!(F3)',195,184,116,true)
						else
							thePlayer:outputChat('['..exports.config:getServerName()..']:#D0D0D0 Bir hata meydana geldi!',195,184,116,true)
						end
					else
						thePlayer:outputChat('['..exports.config:getServerName()..']:#D0D0D0 Birlik adı kullanılıyor!',195,184,116,true)
					end
				else
					thePlayer:outputChat('['..exports.config:getServerName()..']:#D0D0D0 Birlik oluşturmak için 10.000'..exports.config:getServerMoneyType()..' paranızın olması gerek!',195,184,116,true)
				end
			else
				thePlayer:outputChat('['..exports.config:getServerName()..']:#D0D0D0 Birlik oluşturmak için var olan birliğinizden çıkmalısınız!',195,184,116,true)
			end
		else
			thePlayer:outputChat('['..exports.config:getServerName()..']:#D0D0D0 /'..commandName..' [Birlik Adı]',195,184,116,true)
		end
	end
end)